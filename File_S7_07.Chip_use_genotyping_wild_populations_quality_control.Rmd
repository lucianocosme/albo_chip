...
We can analyse the LD patterns for each populations using different approaches. If we want to look
at general patterns we can get an estimate for each population, for example, the half of the
distance of the r2 max. We can use Plink2 to estimates some statistics for us. One important
consideration is how good is our genome assembly. Does it matter if we use the 574 scaffolds or
assign each scaffold to a chromosome or not. So, if you want to know if it matters, you would need
to create a new chromosomal scale and perform the calculations. Since, we want to get only a
general approximate estimate of the linkage levels, we can look at the largest scaffolds. Since
chromosome 1 has the M locus, we will get the largest scaffold of chromosome 2 and 3. We create a
new chromosomal scale when we want to plot the data. For now, we can import a file that I created
with the size of each scaffold (data/genome/scaffold_sizes.txt). The code below imports the file and
gets the 3 largest scaffolds from each chromosome.
...
snps <-
  read_delim(                    # to learn about the options use here, run ?read_delim on the console.
    here(
      "output", "populations", "file4.bim" #output/populations/file4.bim
...
We have 3582 SNPs to estimate LD that are located in the two largest scaffolds of chromosome 2 and 3.
```{r save_list_snps_4LD}
#   ____________________________________________________________________________
#   save calculation to load later                                          ####
write.table(
  snps_4ld,
  file      = here(
    "output", "populations", "snps_4ld.txt"
  ),
  sep       = "\t",
  row.names = FALSE,
  col.names = FALSE,
...
Now we have to select the populations for which we want to estimate the LD statistic. Lets only estimate LD for populations that we have 12 or more individuals.

```{bash get_pops_n2, cache=TRUE}
awk '{print $1}' output/populations/file4.fam | sort | uniq -c | awk '{print $2, $1}' | awk '$2 >= 12 {print}'
```
...
```{bash create_directory_pop_LD_estimates, eval=FALSE}
# make directory
mkdir -p output/populations/ld
```

Create list of populations
```{bash create_list_pops_LD}
awk '{print $1}' output/populations/file4.fam | sort | uniq -c | awk '{print $2, $1}' | awk '$2 >= 12 {print}' | awk '{print $1}' > output/populations/ld/pops_4ld.txt
```
...
--allow-extra-chr \
--keep-allele-order \
--extract output/populations/snps_4ld.txt \
--bfile output/populations/file4 \
--keep-fam /dev/stdin \
--maf 0.1 \
--r2 \
--out output/populations/ld/$fam \
--geno 0.2 \
--write-snplist \
--silent
done;
#
rm output/populations/ld/*.nosex
```

I like to use only variants that are present in at least 50% of the populations. Once we subset the bed files per population, we set a MAF and genotype missingness of 10% as before but now we have a smaller sample size and not all populations will have all the variants.
```{python}
import os

# Replace with the directory containing the .snplist files
directory = "output/populations/ld"
...
with open("output/populations/ld/all_unique_variants_50pct.txt", "w") as file:
    file.write("\n".join(all_variants))
```

Check how many SNPs
```{bash}
cat output/populations/ld/all_unique_variants_50pct.txt | sort | awk '!seen[gensub(/^[[:space:]]+|[[:space:]]+$/, "", "g")]++' | wc -l
```

Now we can estimate LD again using the 5,057 variants that are present in at least 50% of the populations.
...
--allow-extra-chr \
--keep-allele-order \
--extract output/populations/ld/all_unique_variants_50pct.txt \
--bfile output/populations/file4 \
--keep-fam /dev/stdin \
--maf 0.1 \
--r2 \
...
```{bash}
head output/populations/ld/OKI.ld
```

Clean the LD files from Plink
```{r}
ld_files <- list.files(path = "output/populations/ld", pattern = "\\.ld$", full.names = TRUE)
...
Lets check one of the files
```{bash}
head output/populations/ld/KAG.ld
```
...
dump(                                                     # check ?dump for more information
  ...
  here(
    "scripts", "analysis", "import_ld_files.R")       # use here to save it our function as .R 
)
```

Import the LD data
```{r, message=FALSE}
my_ld_tibbles <- import_ld_files("output/populations/ld")


head(my_ld_tibbles$KAG)
```
...
# Calculate the half distance of max r^2 value for each tibble and store in the data.frame
...
ggsave(
  here(
    "output", "populations", "figures", "linkage_half_distance_LONG_scaffolds.pdf"
  ),
  width  = 12,
  height = 6,
  units  = "in"
...
```{bash, eval=FALSE}
mkdir -p output/populations/ld/blocks
```
...
--blocks no-pheno-req \
--blocks-max-kb 200 \
--out output/populations/ld/blocks/$fam \
--geno 0.1 \
--silent
done;
#
rm output/populations/ld/blocks/*.nosex
```

Now we can get some data from our .log files
```{bash}
echo "Population,n,nVariants,geno,maf,passQC" > output/populations/ld/blocks/table_ld_stats.csv
for file in output/populations/ld/blocks/*.log
do
  ...
  echo "$filename,$n,$variants,$geno,$maf,$pass" >> output/populations/ld/blocks/table_ld_stats.csv
done;
head -n 5 output/populations/ld/blocks/table_ld_stats.csv
```

We can check it out 
```{r check_stats_ld_blocks}
# Load data from the table_ld_stats.csv file
ld_blocks <- read.csv(
  here(
    "output", "populations", "ld", "blocks", "table_ld_stats.csv"
    ),
  header = TRUE,
  sep = ","
)
...
officer::read_docx() |>
  ...
  print(target = here::here("output", "populations", "summary_maf_geno_filter_population.docx"))

ft
```

Now we can count how many blocks we found in each population
```{bash parse_ld_blocks_logs}
wc -l output/populations/ld/blocks/*.blocks | \
  awk '{population = gensub(/\.blocks/, "", "g", $2); print population "\t" $1}' | \
  sed 's#output/populations/ld/blocks/##' | \
  sed '$d' > output/populations/ld/blocks/populations_block_counts.csv;
head -n 30 output/populations/ld/blocks/populations_block_counts.csv
```
...
officer::read_docx() |>
  body_add_flextable(ft) |>
  print(target = here::here("output", "populations", "summary_ld_blocks.docx"))

ft
```

Get the size of each block from the .block.det files
```{r estimate_size_ld_block}
...
dir_path <- here("output", "populations", "ld", "blocks")
...
ggsave(
  here(
    ...
    "populations",
    "figures",
    "block_density_y_scale_fixed.pdf"
  ),
  width = 10,
  height = 10,
  units = "in"
